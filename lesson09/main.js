...
for (const item of coursesAndDurationArray) {
    ...
    div2.innerHTML = `<p>${item.title} - ${item.monthDuration}</p>`
...
for (const item of simpsons) {
    ...
    for (const itemKey in item) {
        ...
        if (itemKey !== 'photo') {
            description.innerHTML = `${item[itemKey]}`
            divBox.appendChild(description);
        } else {
            let img = document.createElement('img');
            img.src = item[itemKey];
...
for (const item of coursesArray) {
    ...
    monthDuration.innerText = `month duration: ${item.monthDuration}m`;

    let hourDuration = document.createElement('p');
    hourDuration.classList.add('hourDuration');
    hourDuration.innerText = `hour duration: ${item.hourDuration}h`;
